...
export default function ConclusionGraphic() {
  ...
  const nextCard = () => {
    if (currentCard + 1 < conclusion.length) setCurrentCard(currentCard + 1);
  };

  const previousCard = () => {
    if (currentCard - 1 >= 0) setCurrentCard(currentCard - 1);
  };
  ...
  let buttons = () => {
    return (
      <div className="horizontal">
        ...
        <button
          disabled={currentCard === conclusion.length - 1}
